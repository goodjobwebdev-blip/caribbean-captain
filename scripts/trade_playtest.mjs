...
try{
 ...
 for(const cash of [800,3000])for(const from of PORTS)for(const to of PORTS.filter(p=>p.id!==from.id)){
  ...
  for(const good of ALL_GOODS.filter(id=>id!=='provisions')){
   ...
   const q=quoteBasket(g,[sell(good,lo)]);if(q.errors.length)continue;
   g=deal(g,[sell(good,lo)]);
   const net=g.silver-cash-(120-g.provisions)*1.15;
   candidates.push({good:CATALOGUE[good].name,quantity:lo,net:+net.toFixed(2),hours:g.hours-8});
  }
 ...
 const data={assumptions:'Normal winds, no pirates, starting sloop and crew, 100 silver purchase reserve, finite stocks, real actions and skill/event rules. One-way net values subtract provision consumption at 1.15 silver/unit. Routes use perfect knowledge for balance testing, not player-visible forecasts. Repeated route refills to 120 provisions each departure. Upgrade threshold is Merchant Schooner exchange plus 1000 cash reserve; the simulation keeps the sloop.',routes:results,legs,upgradeLeg};
