import {BATTERIES,gunFit,gunDamage,gunAccuracy,shipProtection,sailStats} from '../ships';
...
export function plannerPayload(b:Battle){
 ...
 return {rules_version:'battle-2-outfitting',phase:b.phase,budget:1000-b.unit,npc:{objective:({Merchant:'Escape with cargo',Local:'Survive and disengage',Courier:'Protect passengers and documents',Navy:'Enforce faction law',Privateer:'Capture legal prizes',Pirate:'Disable and capture valuable targets','Bounty Hunter':'Capture captain alive'}[b.npc.role]),role:b.npc.role,temperament:b.npc.temperament,rank:rank(b.npc.level),planningRank:rank(level)},own,ownOutfit:{protection:shipProtection(own.ship),sails:sailStats(own.ship),batteries:Object.fromEntries(BATTERIES.map(battery=>[battery,{...gunFit(own.ship,battery),damageMultiplier:gunDamage(own.ship,battery),accuracyBonus:gunAccuracy(own.ship,battery,distance(own,enemy))}]))},enemy:publicEnemy,wind:b.wind,windStrength:b.windStrength,range:range(distance(own,enemy)),courseForecast:b.phase==='naval'?coursePreview(b,b.npcId,[]):undefined,hazards:b.hazards,round:b.round,rounds:b.rounds,control:b.control,initiative:b.duel?.initiative,exchange:b.duel?.exchange,
