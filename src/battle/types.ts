...
export type Skill='training'|'sailing'|'lookout'|'deception'|'diplomacy'|'intimidation'|'boarding'|'aiming'|'reloading'|'demolitions'|'carpentry'|'sailmaking'|'doctoring'|'leadership'|'lightWeapons'|'mediumWeapons'|'heavyWeapons'|'athletics'|'shooting';
...
export type CrewDomain='sailing'|'gunnery'|'fighting';
export type Crew={expertise?:Partial<Record<CrewDomain,number>>;unpaidWageHours?:number;fit:number;injured:number;dead:number;morale:number;discipline:number;experience:number;equipment:number};
...
export type Npc={silver?:number;role:Role;faction:Nation;temperament:Temperament;level:number;origin:PortId;departed:number;market:Snapshot;ship:OwnedShip;crew:Crew;perceivedStrength:number};
...
export type Encounter={practice?:Partial<Record<Skill,number>>;stage:'sighting'|'response'|'pursuit'|'counteroffer';npc:Npc;posture:Posture;demand?:Demand;responses:number;early:boolean;movementUsed:boolean;passengersRefused?:boolean;message:string};
...
export type Battle={playerArmour?:{points:number;remaining:number;dodgePenalty:number};crewPractice?:Partial<Record<CrewDomain,number>>;crewResolved?:boolean;practice?:Partial<Record<Skill,number>>;practiceAwarded?:boolean;settlement?:import('./capture').CaptureSettlement;id:string;phase:'naval'|'deck'|'duel'|'capture'|'ended';playerId:string;npcId:string;ships:Record<string,Combatant>;npc:Npc;window:number;unit:number;wind:number;windStrength:number;hazards:{id:string;x:number;y:number;owner:string;armed:boolean}[];plans:Record<string,Scheduled[]>;committed?:string[];accepted?:Decision;decisionKey:number;status:'planning'|'waiting'|'reveal'|'playback'|'replacement'|'transition'|'finished';replacement?:string[];round:number;rounds:number;control:number;attacker:string;duel?:Duel;winner?:string;reason?:string;terms?:string;preloaded:string[];log:string[];rolls:Roll[];difficulty:'Easy'|'Normal'|'Hard';deception?:Record<string,{window:number;band:number}>;result?:{ships:Record<string,Combatant>;winner?:string;reason:string;terms?:string}};
...
export const crewExperience=(c:Crew,domain:CrewDomain)=>clamp(c.expertise?.[domain]??c.experience,0,100);
...
export function strength(s:Combatant){const c=s.crew;return c.fit*(.5+(crewExperience(c,'fighting')+c.morale+c.discipline+c.equipment)/800)*(1+.05*(s.skills.boarding??0));}
