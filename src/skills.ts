...
/** Returns a new skill record; encounter delays and time in port are not practice. */
export function creditSailing(skills:PlayerSkills|undefined,hours:number){
  ...
  const total=(sailing.sailingHours??0)+hours;
  const earned=Math.floor(total/24);
  sailing.sailingHours=total%24;
  sailing.points+=earned;
  ...
  return {skills:next,earned,tiers:sailing.tier-before};
}
