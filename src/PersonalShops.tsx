...
import {APPAREL,TIER_NAMES,apparel,apparelResaleValue,type Apparel,type OutfitSlot} from './apparel';
import {WEAPONS,equipment,appearanceIndex,armourStats,type Equipment} from './equipment';
import {skillName} from './skills';

type Controls={game:Game;busy:boolean;perform:(action:Action)=>void};
const silver=(n:number)=>n.toLocaleString('en');
const CLOTHING=APPAREL.filter(item=>item.kind==='clothing');
const ARMOUR=APPAREL.filter(item=>item.kind==='armour');

function ApparelCard({item,inventory:e,game:g,busy,perform}:{item:Apparel;inventory:Equipment}&Controls){
 const owned=e.wardrobe!.includes(item.id),worn=e.outfit?.[item.slot]===item.id;
 const sale=apparelResaleValue(item);
...
export function Weaver(props:Controls){
 const inventory=equipment(props.game);
 const items=CLOTHING.filter(item=>!item.starter||inventory.wardrobe!.includes(item.id));
 return <>
  <Outfit game={props.game}/>
  <h2>Clothing</h2>
  <p className="muted">Clothes shape your appearance but do not yet change prices or conversations. The weaver pays 15% of the original price for unworn pieces.</p>
  <div className="shop-grid">{items.map(item=><ApparelCard key={item.id} item={item} inventory={inventory} {...props}/>)}</div>
 </>;
}
