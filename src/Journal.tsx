...
import {contractBuilding,cash, date, cargoUsed, passengers, port, GOODS, currentShip, ownedShip, hullPercent, effectiveSpeed, questSkillReward, type Game, type Contract} from './game';
...
function Quest({quest,status,finished}:{quest:Contract;status:string;finished?:number}){return <article className="quest-entry"><div className="section-line"><h3>{quest.type} to {port(quest.to).name}</h3><span className="quest-status">{status}</span></div><p>{port(quest.from).name} → {port(quest.to).name}</p><Details items={[[status==='Completed'?'Payment received':'Payment on delivery',`${cash(quest.reward)} silver`],['Sailing reward',`${finished!==undefined?(quest.sailingReward??0):questSkillReward(quest)} points`],['Load',quest.type==='Freight'?`${quest.amount} hold units`:quest.type==='Passengers'?`${quest.amount} passengers`:'One letter · no hold space'],['Delivery building',contractBuilding(quest)],['Deadline','None']]}/>{finished!==undefined&&<small>Completed {date(finished)}</small>}</article>;}
