...
export function Loadout({game:g,busy,perform}:Controls){
 ...
 {BATTERIES.filter(b=>(g.ship?.cannons[b]??0)>0).map(b=><label key={b}>{skillName(b)} battery · {g.ship!.cannons[b]} × {gunFit(g.ship!,b).calibre}-pound {gunFit(g.ship!,b).type}s<select aria-label={`${b} prepared ammunition`} disabled={busy} value={e.preloads[b]??''} onChange={event=>perform({type:'prepare-battery',battery:b,ammo:(event.target.value||null) as Ammo|null})}><option value="">No preset</option>{AMMUNITION.map(a=><option key={a} value={a}>{a.replaceAll('-',' ')}</option>)}</select></label>)}
