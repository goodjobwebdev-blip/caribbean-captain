...
it('renders shipyard offers and the Journal for a purchased configuration',()=>{
 ...
 expect(journal).toContain('Universal Schooner');expect(journal).toContain('1.45 distance units/hour');expect(journal).toContain('110 / 110');expect(journal).toContain('120.0 / 240');expect(journal).toContain('9');expect(journal).toContain('Sailing performance');expect(journal).toContain('Hull protection reduces incoming naval damage');
