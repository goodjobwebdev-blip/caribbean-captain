...
describe('sailing and economy',()=>{
 ...
 it('resumes a saved prototype pirate encounter and waits for a player choice',()=>{const g=legacyVoyage(newGame('Anne',42));expect(g.voyage?.remaining).toBe(49);expect(g.lastRoll?.dice).toEqual([1,1]);expect(g.port).toBe('bridgetown');expect(()=>act(g,{type:'repair'})).toThrow('encounter');const next=act(g,{type:'encounter',choice:'flee'},random(.99,.99));expect(next.port).toBe('saint-pierre');expect(next.voyage).toBeNull();expect(next.ship?.hullPoints).toBe(100);});
 it('applies different encounter risks in each outcome band',()=>{const attack=legacyVoyage(newGame('Anne',42));const fled=act(attack,{type:'encounter',choice:'flee'},random(0,0));expect(fled.ship?.hullPoints).toBe(75);expect(fled.hours).toBe(81);const fought=act(attack,{type:'encounter',choice:'fight'},random(.5,.5));expect(fought.ship?.hullPoints).toBe(80);expect(fought.crew).toBe(9);const won=act(attack,{type:'encounter',choice:'fight'},random(.99,.99));expect(won.ship?.hullPoints).toBe(95);expect(won.crew).toBe(10);expect(won.silver).toBeGreaterThan(fought.silver);});
 it('fails irreversibly without a rescue when ship is lost',()=>{const g=newGame('Anne',42);g.ship!.hullPoints=30;const attack=legacyVoyage(g);const loss=act(attack,{type:'encounter',choice:'fight'},random(0,0));expect(loss.ship?.hullPoints).toBe(0);expect(loss.failed).toContain('ship was lost');expect(()=>act(loss,{type:'buy',good:'provisions',quantity:10})).toThrow('checkpoint');});
 ...
 it('restores ship condition through one repair and charges time',()=>{const g=newGame('Anne',42);g.ship!.hullPoints=60;const next=act(g,{type:'repair'});expect(next.ship?.hullPoints).toBe(100);expect(next.hours-g.hours).toBe(8);expect(next.silver).toBeCloseTo(800-80-20/3);});
...
describe('Sailing mastery speed bonus',()=>{
 ...
 it('keeps old profiles at baseline and does not recalculate an already-started voyage',()=>{
  ...
  const encounter=legacyVoyage(legacy);
