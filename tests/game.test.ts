...
describe('sailing and economy',()=>{
 ...
 it('fails irreversibly without a rescue when ship is lost',()=>{const g=newGame('Anne',42);g.ship!.hullPoints=30;const attack=act(g,{type:'sail',to:'saint-pierre'},random(.5,0,0));const loss=act(attack,{type:'encounter',choice:'fight'},random(0,0));expect(loss.ship?.hullPoints).toBe(0);expect(loss.failed).toContain('ship was lost');expect(()=>act(loss,{type:'buy',good:'provisions',quantity:10})).toThrow('checkpoint');});
